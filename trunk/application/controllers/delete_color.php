...
class Delete_color extends CI_Controller {
	...
	function deletecolor($colorname){
		...
		if(!$this->session->userdata('username'))
			...
			if($this->color_model->delete_color($colorname)){
				redirect('configure');
			}
			else{
				//CREATE HEADER ARRAY
				...
				$this->load->view('content/configuration_view', array('colordeleteerror' => true, 'wikilist' => $this->user_model->get_wiki_list($this->session->userdata('username')), 'colorlist' => $this->user_model->get_color_list($this->session->userdata('username'))));
